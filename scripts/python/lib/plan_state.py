...
def create_plan_state_directory(state_dir: Path) -> None:
...
def seed_plan_state(plan_data: Any, plan_path: Path, state_dir: Path, harness_root: Path | None = None) -> None:
    """Seed the state directory with metadata, main, and step files."""
    # Extract state initialization configuration from plan
    state_init = plan_data.get("state_initialization", {})
    metadata_schema_version = state_init.get("metadata_schema_version", 1)
    require_step_files = state_init.get("require_step_files", True)
    ...
    metadata = {
        ...
        "active_step": plan_data.get("active_step"),
    ...
    metadata_schema_path = harness_root / "skills/plan/schemas/state-metadata.schema.json"
    ...
    main_schema_path = harness_root / "skills/plan/schemas/state-main.schema.json"
    ...
    for step in steps:
        ...
        step_schema_path = harness_root / "skills/plan/schemas/state-step.schema.json"
