...
from lib.runbook_xml import load_runbook, RunbookLoadError


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Initialize a runbook's state directory from a runbook file (v1 JSON or v2 XML format).")
    parser.add_argument("runbook_file", help="Path to the runbook file to validate and seed state from (runbook.json for v1, main.xml for v2).")
    return parser
...
def main() -> int:
    ...
    try:
        ...
        # loader. V2 XML runbooks are validated by XSD and parser-backed
        # invariants in lib.runbook_xml.
        ...
        # .runbooks/<runbook_id>/runbook.json or .runbooks/<runbook_id>/main.xml
        if runbook_path.parent.parent.name != ".runbooks":
            print(
                f"Error: Runbook file {runbook_path} must be located at .runbooks/<runbook_id>/runbook.json or .runbooks/<runbook_id>/main.xml so relative state_dir paths resolve safely.",
                file=sys.stderr,
            )
            return 1
