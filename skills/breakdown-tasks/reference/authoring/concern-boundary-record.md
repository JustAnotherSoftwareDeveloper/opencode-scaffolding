...
Keep one completed record per concern. A reviewer must be able to see one, and only
one, selected disposition for every inventory entry.
...
- **Disposition — select exactly one:**
  - [ ] **Standalone:** INDEPENDENT_RESULT_BASIS
  - [ ] **Prerequisite-linked:** PREDECESSOR_ARTIFACT_AND_ORDER_REASON
  - [ ] **Excluded with basis:** EXCLUSION_AND_SOURCE_BASIS
  - [ ] **Retained coupled:** SHARED_RESULT_VERIFICATION_AND_SEPARATION_RISK
```
...
Select exactly one disposition; do not leave a concern unclassified or mark multiple
outcomes.

- **Standalone** applies when the concern produces its own result and can be
  independently reviewed. Record why the independence and one-result tests pass.
- **Prerequisite-linked** applies when the concern remains separate but needs another
  result first. Record the predecessor, required artifact, and order rationale. A
  dependency explains sequence; it does not combine concerns.
- **Excluded with basis** applies only when the concern will not be retained. Record a
...
- **Retained coupled** applies only when the concerns share one result and one
...
Before moving on, scan the complete inventory: every concern must have its own
visible record and exactly one checked disposition. Use the decomposition method's
boundary mapping and set review for the later cross-record review.
