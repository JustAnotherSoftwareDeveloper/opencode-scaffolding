...
- Refer to `./frontmatter-rules.md` for class taxonomy and platform rules — this guide does not duplicate them.
- This guide applies to `SKILL.md` files only. Support files (`reference/<module>/*.md`) may use looser conventions.
...
- **Target length**: under 200 lines for `SKILL.md`. If content exceeds 200 lines, push depth into `./reference/<module>/*.md`, `./schemas/`, or `./snippets/`.
...
- Push detail into `./reference/<module>/*.md`, `./schemas/`, or `./snippets/` (prose, rules, gotchas, examples). Keep `SKILL.md` procedural only.
...
- Do **not** inline content from `./reference/<module>/*.md` into `SKILL.md`. If a rule belongs in both files, place the authoritative version in a `./reference/<module>/*.md` file and reference it.
