...
- **`operation`** — Broad/default class for single bounded procedures that are independent, self-validating, and do not sub-delegate.
...
- **`orchestrated`** — Coordinates sub-skills, workers, phases, or quality gates.
  Orchestrated skills use the 7-section canonical layout (Frontmatter, Purpose/Intro, Execution Steps, Worker Strategy, Verification Checklist, Self-Validation, Cross-References).
  See `../templates/orchestrated.SKILL.template.md` for the canonical skeleton.
...
- **`documentation`** — Passive data store for domain-shared reference content (docs, schemas, templates).
  No side effects, no execution steps.
  Consumed by other skills via relative-path references.
  See `../templates/documentation.SKILL.template.md` for the canonical skeleton.
...
Start with operation as the default; only choose another class when a specific condition clearly applies:
...
- **Passive data store for shared docs/schemas/templates consumed by other skills?** → `documentation`
...
- A documentation skill is a passive data store consumed by other skills via relative-path references; it must not define execution steps or produce side effects.
