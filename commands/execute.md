...
4. Initialize state with the selected source: `uv run --project scripts/python init-runbook-state .runbooks/<runbook_id>/main.xml` for v2, or `.runbooks/<runbook_id>/runbook.json` for legacy v1.
...
- Prefer v2 `main.xml`; use `runbook.json` only as legacy compatibility.
