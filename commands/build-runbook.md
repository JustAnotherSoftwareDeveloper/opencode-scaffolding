...
Generate a v2 TOON runbook workspace from a plan slug. If no slug is provided, use the most recent plan in `.plans/`.
...
4. Create `.runbooks/<unix-timestamp>-slug/main.toon` plus one `steps/<step-id>.toon` file per executable step.
5. Validate the v2 runbook workspace: `uv run --project scripts/python validate-runbook .runbooks/<runbook_id>/main.toon`.
...
- Use v2 TOON by default. Legacy `.runbooks/<id>/runbook.json` creation is only for explicit v1 compatibility requests.
