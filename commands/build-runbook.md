...
Generate a runbook from a plan slug. If no slug is provided, use the most recent plan in `.plans/`.
...
4. Create `.runbooks/<unix-timestamp>-slug/runbook.json` from the plan.
5. Validate the runbook manifest: `uv run --project scripts/python validate-json .runbooks/<runbook_id>/runbook.json --schema skills/runbook/schema.json`.
